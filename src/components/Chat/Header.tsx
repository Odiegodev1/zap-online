...
export const Header = () => {
    ...
    const userIsOnline = dayjs().subtract(5, 'minutes').isBefore(dayjs(chats?.find(item => item.id === chat?.id)?.user.last_accsess))
        ...
        <div className="flex items-center gap-6 border-b bg-slate-100/80 dark:bg-slate-900/80 px-8 pr-12 h-16">
            ...
            <div className="flex-1">
                ...
                <small className="text-slate-500 dark:text-slate-400 block mt-0.5">
                    {userIsOnline ? "Online" : `Última visualização ${dayjs(chats?.find(item => item.id === chat?.id)?.user.last_accsess).format('DD/MM/YYYY [ás] HH:mm')}`}
                </small>
            </div>
