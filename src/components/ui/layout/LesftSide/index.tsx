...
import { Chat, UpdateChatEvent } from "@/app/types/Chat"
...
export const LeftSide = ({ variant = "desktop" }: Props) => {
    ...
    return (
        <div className={`bg-slate-100 dark:bg-slate-900 border-r border-slate-50 dark:border-slate-800 ${variant === "mobile" ? 'w-auto' : 'w-96'} h-screen overflow-auto`}>
            <NewChat />
            ...
            <div className="mt-5">
                {chatsFiltered.map(chat => (
                    ...
                    >
                        <Avatar className="size-[46px]" isOnline={dayjs().subtract(5, 'minutes').isBefore(dayjs(chat.user.last_accsess))}>
                            <AvatarImage
